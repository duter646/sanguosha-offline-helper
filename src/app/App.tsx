...
export function App() {
  ...
  const createPool = () => { const name = poolNameDraft.trim(); if (!name) return; if (pools.some((poolItem) => poolItem.name === name)) { setPoolManagerMessage('将池名称已存在，请换一个名称'); return } const id = `pool-${Date.now()}`; updatePools([...pools, { id, name, heroIds: [] }]); selectPool(id) }
  const duplicatePool = () => { if (!activePool) return; const name = poolNameDraft.trim() || `${activePool.name} 副本`; if (pools.some((poolItem) => poolItem.name === name)) { setPoolManagerMessage('将池名称已存在，请换一个名称'); return } const id = `pool-${Date.now()}`; updatePools([...pools, { id, name, heroIds: [...activePool.heroIds] }]); selectPool(id) }
