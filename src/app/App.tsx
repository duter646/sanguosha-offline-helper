...
export function App() {
  ...
  useEffect(() => {
    const onPopState = () => {
      ...
      setPage(next === 'heroes' || next === 'pool' || next === 'draw' ? next : 'home')
    }
  ...
  useEffect(() => {
    if (page === 'pool' && activePool) setPoolNameDraft(activePool.name)
  }, [page, activePool?.id])
  ...
  const poolConfig = <section className="content-section pool-page"><div className="section-heading"><div><span className="eyebrow">POOL SETTINGS</span><h2>配置将池</h2></div><span className="count">{poolHeroIds.length} / {heroes.length}</span></div><div className="pool-page__panel"><div className="pool-manager"><div className="pool-manager__title"><strong>管理将池</strong><small>切换、新建或维护多套独立配置</small></div><div className="pool-manager__row"><select aria-label="选择当前将池" value={activePool?.id} onChange={(event) => selectPool(event.target.value)}>{pools.map((poolItem) => <option key={poolItem.id} value={poolItem.id}>{poolItem.name} · {poolItem.heroIds.length} 将</option>)}</select><input value={poolNameDraft} onChange={(event) => { setPoolNameDraft(event.target.value); setPoolDeletePending(false); setPoolManagerMessage('') }} placeholder="输入名称后可重命名" aria-label="将池名称" /><button onClick={createPool}>新建</button><button disabled={!activePool} onClick={duplicatePool}>复制</button><button disabled={!activePool || !poolNameDraft.trim()} onClick={renamePool}>重命名</button><button disabled={!activePool || activePool.isDefault || pools.length <= 1} onClick={deletePool}>{poolDeletePending ? "确认删除" : "删除"}</button></div>{poolManagerMessage && <small className="pool-manager__message">{poolManagerMessage}</small>}</div><div className="pool-config__header"><strong>当前将池 · {activePool?.name} · {poolHeroIds.length}/{heroes.length}</strong><span><button onClick={() => updatePoolHeroes(heroes.map((hero) => hero.id))}>全选</button><button onClick={() => updatePoolHeroes([])}>全不选</button></span></div><div className="pool-search"><input value={poolQuery} onChange={(event) => { setPoolPage(1); setPoolQuery(event.target.value) }} placeholder="搜索武将或技能" /></div><div className="pool-list">{pagedPoolHeroes.map((hero) => <label className="pool-item" key={hero.id}><input type="checkbox" checked={poolHeroIds.includes(hero.id)} onChange={() => updatePoolHeroes(poolHeroIds.includes(hero.id) ? poolHeroIds.filter((id) => id !== hero.id) : [...poolHeroIds, hero.id])} /><span>{hero.name}</span><small>{skills.filter((skill) => hero.skillIds.includes(skill.id)).map((skill) => skill.name).join('、')}</small></label>)}<div className="pagination"><button disabled={poolPage === 1} onClick={() => setPoolPage((current) => current - 1)}>上一页</button><span>{poolPage} / {poolPageCount}</span><button disabled={poolPage >= poolPageCount} onClick={() => setPoolPage((current) => current + 1)}>下一页</button></div></div></div></section>
  return <div className="app-shell"><header className="topbar"><button className="brand" onClick={() => setPage('home')}><span>一将成名</span><small>线下辅助</small></button><nav aria-label="主导航"><button className={page === 'home' ? 'active' : ''} aria-current={page === 'home' ? 'page' : undefined} onClick={() => setPage('home')}>工具首页</button><button className={page === 'heroes' ? 'active' : ''} aria-current={page === 'heroes' ? 'page' : undefined} onClick={() => setPage('heroes')}>武将查询</button><button className={page === 'pool' ? 'active' : ''} aria-current={page === 'pool' ? 'page' : undefined} onClick={() => setPage('pool')}>配置将池</button></nav></header><main>
    {page === 'home' && <section className="tool-home"><div className="tool-home__header"><div className="tool-home__intro"><span className="eyebrow">SKILL TOOLKIT</span><h1>选择技能工具</h1><p>选择武将，开始对应的线下技能辅助。</p></div><label className="tool-home__pool"><span>当前将池 · {poolHeroIds.length}/{heroes.length}</span><select value={activePool?.id} onChange={(event) => selectPool(event.target.value)}>{pools.map((poolItem) => <option key={poolItem.id} value={poolItem.id}>{poolItem.name}</option>)}</select></label></div><div className="tool-grid"><div className="tool-grid__meta">{toolHeroes.length} 个工具 · 常用优先</div><div className="hero-grid">{toolHeroes.map((hero) => <button className="tool-card" key={hero.id} onClick={() => { recordToolUse(hero.id); resetTool(); setDrawHeroId(hero.id); setPage('draw') }}><span className="hero-faction">{hero.faction}</span><strong>{hero.name}</strong><span>{skills.find((skill) => hero.skillIds.includes(skill.id))?.name}</span><small>进入工具 →</small></button>)}</div></div></section>}
    {page === 'pool' && poolConfig}
